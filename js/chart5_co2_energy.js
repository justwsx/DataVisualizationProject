 ...
 * --------------------------------------------------------------------------
 * Renders a Scatter Plot (Bubble Chart) visualizing the relationship between:
 * - X-Axis: Primary Energy Consumption (Log Scale).
 * - Y-Axis: Carbon Intensity (% of Fossil Fuels).
 * - Bubble Size: Population.
 * - Color: Carbon Intensity (Green = Clean, Red = Dirty).
 */
class CO2EnergyChart {
    ...
    constructor(data) {
        this.data = data;
        this.elementId = "chart-co2-energy"; // Target DOM ID
        this.year = 2020;
        ...
        // Visual constants
        this.maxR = 45; // Max bubble radius
        this.clipPad = this.maxR + 2; // Padding to prevent bubble clipping
        this.domainPadFactor = 1.35; // Padding for axis domains

        // Color Scale: Green (Low Carbon) -> Yellow -> Red (High Carbon)
        ...
        // Responsive Resizing Logic (Debounced via RAF)
    ...
    /**
     * Updates the chart to a specific year.
     * @param {number} year - The year to display.
     * @param {Object} opts - Options (e.g., animation duration).
     */
    update(year, opts = {}) {
        this.year = year;
    ...
    /**
     * Re-draws the chart immediately (used for resizing).
     */
    resize() {
    ...
    /**
     * Cleanup method to remove observers and DOM elements.
     */
    destroy() {
    ...
    /**
     * Calculates global min/max values across the ENTIRE dataset.
     * This ensures the axes remain stable when switching years.
     */
    _computeGlobalDomains() {
        ...
        // Add padding to domains for visual comfort
    ...
    _initOnce() {
        ...
        // 1. Clip Path (keeps bubbles inside the plot area)
        this.clipPath = this.defs.append("clipPath")
            .attr("id", this.clipId)
            .attr("clipPathUnits", "userSpaceOnUse");
        this.clipRect = this.clipPath.append("rect");

        // 2. Linear Gradient for Legend/Color Scale
        ...
        // 3. Layer Groups (Order determines Z-index)
    ...
    /**
     * Data Processing:
     * Calculates 'Carbon Intensity' (percentage of energy from fossil fuels).
     * Filters valid data points for the current year.
     */
    _getYearData() {
        ...
        return yearData.map(d => {
            ...
            // Method A: Direct Fossil Fuel data available
            ...
            // Method B: Summation of individual sources (fallback)
            else {
                const fossil =
        ...
        // Sort by population descending so small bubbles render on top of large ones
        .sort((a, b) => b.population - a.population);
    }

    /**
     * Main Rendering Loop.
     * Handles dimensions, axes, and D3 Data Joins.
     */
    draw({ duration = 350 } = {}) {
        const container = document.getElementById(this.elementId);
        ...
        // 2. Define Scales
        const x = d3.scaleLog().domain(this.xDomain).range([0, w]);
        const y = d3.scaleLinear().domain(this.yDomain).range([h, 0]);
        const r = d3.scaleSqrt().domain(this.popDomain).range([3, this.maxR]);

        // 3. Prepare Data
        const traceData = this._getYearData();

        this.bubblesG.selectAll("circle").interrupt(); // Stop active animations
        ...
        this.annG.append("text")
            .attr("x", w - 140).attr("y", h - 20)
            .text("Lower % = Cleaner Energy")
            .style("font-size", "11px").attr("fill", "#059669");

        // 7. Render Bubbles (Enter/Update/Exit)
        ...
        circles.join(
            enter => enter.append("circle")
                ...
                .attr("r", d => r(d.population)), // Grow to size
            update => update
                .transition().duration(dur)
                .attr("cx", d => x(d.energy))
                .attr("cy", d => y(d.intensity))
    ...
    /**
     * Renders the vertical gradient legend on the right side.
     */
    _drawColorBar(w, h) {
        ...
        legendG.append("text")
            .attr("x", 0).attr("y", -10)
            .text("Intensity")
            .style("font-size", "10px").attr("fill", "#475569").style("font-weight", "bold");
