...
// Aggiungi questo nel metodo setupEventListeners() o a fondo file
const initSidebarToggle = () => {
    ...
    // AUTO-HIDE: Chiudi la barra automaticamente quando si clicca un link (Overview, Demand, ecc.)
    navLinks.forEach(link => {
        link.addEventListener('click', () => {
            // Nascondi sidebar su schermi piccoli o se vogliamo focus totale
            if (window.innerWidth < 1200) {
                dashboard.classList.add('sidebar-collapsed');
                toggleBtn.querySelector('.material-icons-outlined').textContent = 'menu';
            }
        });
    });
};

// Avvialo dopo che il DOM è pronto
document.addEventListener('DOMContentLoaded', () => {
    Dashboard.init();
    initSidebarToggle();
});
